        ...
        if (W) {
            ...
            switch(datasize_mux(get_DATASIZE(CURRENT_LATCHES.MICROINSTRUCTION), mask_val(CURRENT_LATCHES.IR, 14 ,12), 0)){
                ...
                case -2: //16bit
                    ...
                    break;
                case -3 : //32bit
                    MEM_VAL = sext_unit((MEMORY[CURRENT_LATCHES.MAR+3] << 24) + (MEMORY[CURRENT_LATCHES.MAR+2] << 16) + (MEMORY[CURRENT_LATCHES.MAR+1] << 8) + MEMORY[CURRENT_LATCHES.MAR], 32);
                    break;
                case 0:
                    MEM_VAL = (MEMORY[CURRENT_LATCHES.MAR+3] << 24) + (MEMORY[CURRENT_LATCHES.MAR+2] << 16) + (MEMORY[CURRENT_LATCHES.MAR+1] << 8) + MEMORY[CURRENT_LATCHES.MAR];
                    break;
                default:
                    error("INVALID DATASIZE %d\n", datasize_mux(get_DATASIZE(CURRENT_LATCHES.MICROINSTRUCTION), mask_val(CURRENT_LATCHES.IR, 14 ,12), 0));
            }
